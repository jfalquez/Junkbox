...
#include "DTrackVicon.h"


// Global CVars
DTrackViconConfig         dtvConfig;


///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
DTrackVicon::DTrackVicon( unsigned int nImageWidth, unsigned int nImageHeight ) :
...
{
    mvl::PrintHandlerSetErrorLevel( dtvConfig.g_nErrorLevel );
    ...
    dtvConfig.g_vPyrMaxIters.resize( MAX_PYR_LEVELS );
    dtvConfig.g_vPyrMaxIters.setZero();
    dtvConfig.g_vPyrMaxIters << 1, 1, 2, 2, 5;

    // initialize if full estimate should be performed at a particular level
    // 1: full estimate          0: just rotation
    dtvConfig.g_vPyrFullMask.resize( MAX_PYR_LEVELS );
    dtvConfig.g_vPyrFullMask.setZero();
    dtvConfig.g_vPyrFullMask << 1, 1, 1, 1, 0;
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
DTrackVicon::~DTrackVicon()
{
}

///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool DTrackVicon::Init(
    ...
    )
{
    ...
    if( dtvConfig.g_bConstantVelocityMotionModel == true ) {
        T_w_c = m_T_p_c;
    }
    ...
    m_pMap->FindClosestKeyframes( T_w_c, dtvConfig.g_fCloseKeyframeNorm, vNearKeyframes );
    ...
    if( vNearKeyframes.empty() == false ) {
        ...
        double dSADThreshold = dtvConfig.g_nLoopClosureSAD * (m_nThumbHeight * m_nThumbWidth);
    ...
    if( pClosestKeyframe == NULL || pLastKeyframe == pClosestKeyframe ) {
        ...
        if( abs(nFrameId - nClosestKeyframeId) > dtvConfig.g_nLoopClosureMargin ) {
            // frame is "distant" enough that we might consider it a valid loop closure
            if( dErrorLastKeyframe < dtvConfig.g_dLoopClosureThreshold && dErrorClosestKeyframe < dtvConfig.g_dLoopClosureThreshold ) {
    ...
    if( dErrorLastKeyframe < dErrorClosestKeyframe || dtvConfig.g_bAlwaysUseLastKeyframe ) {
...
//        T_k_c.setIdentity();
    ...
    m_Analytics["Num Obs"] = std::pair<double, double>( dObsPts, dtvConfig.g_fKeyframePtsThreshold );
    if( dObsPts < dtvConfig.g_fKeyframePtsThreshold ) {
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
double DTrackVicon::_EstimateRelativePose(
        ...
        )
{
    ...
    for( int PyrLvl = MAX_PYR_LEVELS-1; PyrLvl >= 0; PyrLvl-- ) {
        dLastError = DBL_MAX;
        for(int ii = 0; ii < dtvConfig.g_vPyrMaxIters[PyrLvl]; ii++ ) {
...
// Show DEBUG image from minimization
            ...
            if( dtvConfig.g_vPyrFullMask(PyrLvl) != 0 ) {
                Eigen::FullPivLU< Eigen::Matrix<double,6,6> >    lu_JTJ(LHS);

                // check degenerate system
                if( lu_JTJ.rank() < 6 ) {
                    PrintMessage( dtvConfig.g_nDebugGN, "warning(@L%d I%d) LS trashed. Rank deficient!\n", PyrLvl+1, ii+1 );
                    continue;
                }
            ...
            if( ( fabs( dNewError - dLastError ) < ui_fBreakErrorThreshold ) && ui_bBreakEarly ) {
                PrintMessage( dtvConfig.g_nDebugGN, "notice: Breaking early @ L%d : I%d ...\n", PyrLvl+1, ii+1 );
                break;
            }
