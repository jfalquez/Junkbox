...
m_nPrevTimeStampDepth(0),
m_dPrevTimeStampWrite(0), m_dSumTimeWrite(0),
m_bRun(false), m_nCount(0), m_nSleepMS(30),
m_sPath(sPath),
m_sPrefixLeft(sPrefixLeft), m_sPrefixRight(sPrefixRight),
m_sSuffix(sSuffix),
m_sCameraType(""), m_bConvertMono(bConvertMono),
...
m_sPrefixLeft(sPrefixLeft), m_sPrefixRight(sPrefixRight),
m_sSuffix(sSuffix),
m_sCameraType(sCameraType), m_bConvertMono(bConvertMono),
m_nMaxBufferSize(nMaxBufferSize), m_nDroppedFrames(0),
        m_pCamera(0),
m_dInitTime(Tic()) {
}
...
bool DiskLogger::Start(double dDelayMS) {
    if (m_bRun) {
        Stop();
    }
    ...
    if( m_pCamera == NULL) {
        ...
        if (m_pCamera == NULL ||
                !m_pCamera->InitDriver(m_sCameraType)) {
            return false;
        }
    }
...
void DiskLogger::_GetImageAndQueue() {
    ...
    while (m_bRun) {
        ...
        ssFileNameLeft << m_sPath << "/" << m_sPrefixLeft << "_"
                << std::setw(g_nPaddingImageNumber)
                << std::setfill('0') << m_nCount;
        std::string left = ssFileNameLeft.str();

        std::stringstream ssFileNameRight;
        ssFileNameRight << m_sPath << "/" << m_sPrefixRight << "_"
                << std::setw(g_nPaddingImageNumber)
                << std::setfill('0') << m_nCount;
        ...
        if (m_qImages.size() > m_nMaxBufferSize) {
            ...
            if (vImages.size() == 2) {
...

                {
                    ...
                    m_qImages.push_back
                            (ImagePair
                            (ImageAndName(im1, ssFileNameLeft.str()),
                ...
                m_nCount--;
                cerr << "ERROR: expecting 2 images: RGB and depth, got: "
                        << vImages.size() << " images." << endl;
            }
...
void DiskLogger::_DequeueAndSaveImage() {
    ...
    while (m_bRun) {
        ...
        while (!m_qImages.empty()) {
            ...
            std::cout << m_qImages.size() << " "
                    << std::get < 1 > (std::get < 0 > (ImagePair)) << " "
