...
int main( int argc, char* argv[] )
{
    ...
    octomap::OcTree otree(0.01);  // create empty tree with resolution 0.1
...
//    roo::BoundingBox reset_bb(make_float3(-volrad,-volrad,-volrad), make_float3(volrad,volrad,volrad));
    ...
    roo::Image<float4, roo::TargetHost, roo::Manage> hVbo(w,h);
    ...
    Var<bool> octomap("ui.octomap", false, false);
...
//    pangolin::RegisterKeyPressCallback('s', [&vol,&colorVol]() {roo::SaveMesh("mesh",vol,colorVol); } );
    ...
    pangolin::RegisterKeyPressCallback('o', [&otree]() { otree.writeBinary("otree.bt"); } );
    pangolin::RegisterKeyPressCallback('i', [&otree]() { otree.clear(); } );
    pangolin::RegisterKeyPressCallback('k', [&octomap]() { octomap = true; } );
...
//                        roo::SdfFuse(work_vol, kin_d[0], kin_n[0], T_wl.inverse().matrix3x4(), K, trunc_dist, max_w, mincostheta );
    ...
    //            hVbo.CopyFrom(kin_v[0]);

                for( size_t ii = 0; ii < imgs[1].h; ++ii ) {
                    for( size_t jj = 0; jj < imgs[1].w; ++jj ) {
                        unsigned short* sptr = (unsigned short*)(imgs[1].ptr);
                        int idx = (ii*imgs[1].w)+jj;
                        unsigned short us_depth = *(sptr+idx);
                        float z = us_depth / 1000.0;
                        float x = ((jj-K.u0)/K.fu)*z;
                        float y = ((ii-K.v0)/K.fv)*z;
                        pointCloud.push_back(x, y, z);
    //                    float4 point = hVbo(jj,ii);
    //                    pointCloud.push_back(point.x, point.y, point.z);
                    }
                }

                Eigen::Vector6d cart_pose = T_wl.log();
                octomap::pose6d pose( cart_pose(0), cart_pose(1), cart_pose(2), cart_pose(3), cart_pose(4), cart_pose(5) );
                otree.insertPointCloud(pointCloud, origin, pose);
            }
        }
