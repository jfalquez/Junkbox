...
class Node {
  ...
  */
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  Node(int nPort  //< Input: RPC Port.. randomized in the future
       ) {
    // init context
    m_pContext = new zmq::context_t(1);
    m_pRpcSocket = NULL;
  ...
  static void RPCThreadFunc(Node* pThis) {
    while (1) {
      ...
      std::string FuncName((char*)(ZmqReq.data()) + sizeof(FuncNameSize),
                           FuncNameSize);
  ...
  bool Register(const std::string& sName,          //< Input: Function name
  ...
  bool Call(
      ...
      const google::protobuf::Message&
          MsgReq,  //< Input: Protobuf message containing request
      google::protobuf::Message&
          MsgRep,  //< Output: Protobuf message holding reply
      unsigned int TimeOut =
          100  //< Input: Number of miliseconds to wait for reply
      ) {
    ...
    // prepare message
    zmq::message_t ZmqReq(sizeof(FuncNameSize) + FuncNameSize +
                          MsgReq.ByteSize());
    std::memcpy(ZmqReq.data(), &FuncNameSize, sizeof(FuncNameSize));
    std::memcpy((char*)ZmqReq.data() + sizeof(FuncNameSize), FuncName.c_str(),
                FuncNameSize);
    if (!MsgReq.SerializeToArray(
            (char*)ZmqReq.data() + sizeof(FuncNameSize) + FuncNameSize,
            MsgReq.ByteSize())) {
      // error serializing protobuf to ZMQ message
      return false;
    }
    ...
    // send request
    if (pSock->send(ZmqReq, ZMQ_DONTWAIT) == false) {
    ...
    while (pSock->recv(&ZmqRep, ZMQ_DONTWAIT) == false && nCount < TimeOut) {
      nCount++;
      usleep(1000);
    }
    if (nCount == TimeOut) {
  ...
  bool Publish(
    ...
    if (it != m_mTopics.end()) {
      ...
      try {
        std::cout << "[Node] Publishing topic '" << sTopic << "' on "
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  bool Write(const std::string& sTopic,            //< Input: Topic to write to
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  bool Subscribe(const std::string& sTopic,  //< Input: Topic to subscribe to
                 const std::string& sHost    //< Input: for now, host info..
                                             //later, just the topic
                 ) {
    std::map<std::string, zmq::socket_t*>::iterator it;
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  bool Read(const std::string& sTopic,       //< Input: Topic to read
    ...
    if (it == m_mTopics.end()) {
      ...
      if (pSock->recv(&ZmqMsg, bBlocking == true ? 0 : ZMQ_DONTWAIT) == false) {
        // nothing to read
        return false;
      }
